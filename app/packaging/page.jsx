...
import { Package, Search, RefreshCw, FileText, Camera, Loader2 } from "lucide-react"
...
export default function PackagingPage() {
            ...
            if (data && data.table && data.table.rows) {
                ...
                data.table.rows.slice(6).forEach((row, index) => {
                    ...
                    const order = {
                        ...
                        blColumn: row.c[63]?.f || row.c[63]?.v || null, // Column BL (index 63)
                        bwColumn: row.c[74]?.v || null, // Column BW (index 74)
                        btColumn: row.c[71]?.f || row.c[71]?.v || null,
    ...
    const processPackagingOrder = async (dialogData) => {
        try {
            setUploading(true)
            const {
                order,
            ...
            // Also update Warehouse sheet (Now using updateByOrderNo)
            const formData2 = new FormData()
            formData2.append("sheetName", "Warehouse")
            formData2.append("action", "updateByOrderNo")
            formData2.append("orderNo", order.orderNo)
            formData2.append("totalItems", "0") // Items are usually verified in Sales page
            ...
            if (fileUrls) {
                warehouseRowData[3] = fileUrls.beforePhotoUrl || order.beforePhoto || ""
                warehouseRowData[4] = fileUrls.afterPhotoUrl || order.afterPhoto || ""
                warehouseRowData[5] = fileUrls.biltyUrl || order.biltyUpload || ""
            } else {
            ...
            // Preserve transporter info if already exists
            warehouseRowData[6] = order.transporterName || ""
            ...
            if (!whResult.success && (whResult.error?.includes("not found") || whResult.error?.includes("Order No."))) {
                ...
                formDataInsert.append("totalItems", "0");
    ...
    const handleProcessSubmit = async (dialogData) => {
        const result = await processPackagingOrder(dialogData)
        if (result.success) {
            setIsDialogOpen(false)
            setSelectedOrder(null)
            alert("✅ Packaging documentation COMPLETED!")
        } else {
            ...
            <div className="overflow-x-auto overflow-y-auto max-h-[calc(100vh-320px)] relative w-full scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                <Table className="relative border-separate border-spacing-0">
                    ...
                    <TableBody>
                        {isLoading ? (
                            <TableRow>
                                <TableCell colSpan={showActions ? 11 : 15} className="h-24 text-center">
                            ...
                            <TableRow>
                                <TableCell colSpan={showActions ? 11 : 15} className="h-24 text-center text-muted-foreground">
                                    No orders found.
                                </TableCell>
                            </TableRow>
                        ) : (
                                ...
                                <TableRow key={order.rowIndex} className="hover:bg-slate-50/50 transition-colors text-xs border-b border-slate-100">
                                    ...
                                    {!showActions && (
                                        <>
                                            ...
                                            <TableCell className="border-b border-r border-slate-100">
                                                <Badge
                                                    variant={order.dispatchStatus === "okay" ? "success" : "destructive"}
                                                    className="font-bold text-[10px] h-5"
                                                >
                                                    {order.dispatchStatus === "okay" ? "Okay" : "Not Okay"}
                                                </Badge>
                                            </TableCell>
                                            <TableCell className="text-slate-600 max-w-[200px] truncate border-b border-r border-slate-100" title={order.notOkReason}>
                                                {order.notOkReason || <span className="text-slate-300">-</span>}
                                            </TableCell>
                                        </>
                                    )}
            ...
            <div className="flex flex-col gap-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        <div className="p-2.5 bg-purple-600 rounded-xl shadow-purple-200 shadow-lg text-white">
                ...
                <div className="flex flex-col gap-4">
                    <div className="relative max-w-sm">
                        ...
                        <Input
                            ...
                            className="pl-10 h-11 bg-white border-slate-200 shadow-sm focus:ring-purple-500 focus:border-purple-500 rounded-xl"
                            value={searchTerm}
                    ...
                    <Tabs defaultValue="pending" className="w-full">
                        ...
                        <div className="mt-4">
                            <TabsContent value="pending" className="animate-in fade-in-50 duration-500">
                                <Card className="border-slate-200 shadow-sm overflow-hidden rounded-2xl">
                                    <CardHeader className="bg-slate-50/50 border-b border-slate-100">
                                        <CardTitle className="text-lg font-bold text-slate-800">Ready for Packing</CardTitle>
                                        <CardDescription className="text-slate-500 font-medium">
                                            Orders verified and ready for packaging documentation.
                                        </CardDescription>
                                    </CardHeader>
                            ...
                            <TabsContent value="history" className="animate-in fade-in-50 duration-500">
                                <Card className="border-slate-200 shadow-sm overflow-hidden rounded-2xl">
                                    <CardHeader className="bg-slate-50/50 border-b border-slate-100">
                                        <CardTitle className="text-lg font-bold text-slate-800">Packaging History</CardTitle>
                                        <CardDescription className="text-slate-500 font-medium">
                                            Completed packaging documentation and photo uploads.
                                        </CardDescription>
                                    </CardHeader>
                                    <CardContent className="p-0">
