...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { gradient?: boolean }>(
  ({ className, gradient = false, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("rounded-lg border bg-card text-card-foreground shadow-sm", gradient && "card-gradient", className)}
      {...props}
    />
  ),
)
  ...
  ({ className, gradient = false, ...props }, ref) => (
    <div
      ...
      className={cn(
        ...
        gradient && "bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-t-lg",
        className,
      )}
      {...props}
    />
